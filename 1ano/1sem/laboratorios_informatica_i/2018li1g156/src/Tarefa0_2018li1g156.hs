...
ePosicaoMatrizValida (x,y) (h:t) = eIndiceListaValido y h && eIndiceListaValido x (h:t)
...
criaMatriz (x,y) elem = replicate x $ replicate y elem 
...
atualizaPosicaoMatriz (x,y) newElem m = atualizaIndiceLista x (atualizaIndiceLista y newElem $ encontraIndiceLista x m) m
