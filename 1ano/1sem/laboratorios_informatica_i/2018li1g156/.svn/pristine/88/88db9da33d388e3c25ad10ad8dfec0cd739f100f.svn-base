...
tickLasers :: Estado -- ^ O 'Estado' sem o efeito dos 'DisparoLaser'.
           -> Estado -- ^ O 'Estado' após o efeito dos 'DisparoLaser'.
tickLasers e@(Estado m js []) = e
tickLasers e@(Estado m js ds) = let [ls,os] = lLasers ds in case ls of
                                      [] -> e
                                      _ -> acaoLaser ls m js os

-- | Separa uma lista de 'Disparo' numa lista com lista de 'DisparoLaser' e outra com 'DisparoChoque' e/ou 'DisparoCanhao'.
lLasers :: [Disparo]   -- ^ Lista de 'Disparo' original.
        -> [[Disparo]] -- ^ Lista com as duas listas já distintas.
lLasers [] = [[],[]]
...
-- | Provoca os efeitos dos 'DisparoLaser' no 'Estado'.
acaoLaser :: [Disparo] -- ^ A lista de 'DisparoLaser'.
          -> Mapa      -- ^ O 'Mapa' em contínua atualização.
          -> [Jogador] -- ^ Lista de 'Jogador' do 'Estado' em contínua atualização.
          -> [Disparo] -- ^ A lista de 'Disparo' (sem os 'DisparoLaser') em contínua atualização.
          -> Estado    -- ^ O 'Estado' após o efeito dos 'DisparoLaser'.
...
-- | Dada uma lista sempre com duas 'Posicao', referentes ao locais no 'Mapa' a afetar, destroi, ou não, 'Bloco' 'Destrutivel'.
atualizaBlocos :: [Posicao] -- ^ A lista com as duas 'Posicao'.
               -> Mapa      -- ^ O 'Mapa' do 'Estado'.
               -> Mapa      -- ^ O 'Mapa' atualizado (podendo este não sofrer quaisquer mudanças).
atualizaBlocos [] m = m
...
-- | Dada a 'Posicao' do 'DisparoLaser' em causa e a lista de 'Jogador' do 'Estado', faz as devidas mudanças nesta.                                              
seAtingido :: Posicao   -- ^ A 'Posicao' de um 'DisparoLaser'.
           -> [Jogador] -- ^ A lista de 'Jogador' do 'Estado'.
           -> [Jogador] -- ^ A lista de 'Jogador' atualizada (podendo esta não sofrer quaisquer alterações).
seAtingido pos [] = []
...
-- | Dada a 'Posicao' do 'DisparoLaser' em causa e a lista de 'Disparo', que não sejam 'DisparoLaser', faz as devidas mudanças nesta. 
destroiBala :: Posicao   -- ^ A 'Posicao' de um 'DisparoLaser'.
            -> [Disparo] -- ^ A lista de 'Disparo' inicial. 
            -> [Disparo] -- ^ A lista de 'Disparo' atualizada (podendo esta não sofrer quaisquer alterações).
destroiBala pos [] = []
...
tickCanhoes :: Estado -- ^ O 'Estado' sem o efeito dos 'DisparoCanhao'
            -> Estado -- ^ O 'Estado' com o efeito dos 'DisparoCanhao'
...
-- | Esta função recebe incialmente uma lista com o mesmo 'DisparoCanhao' duplicado, quando o 'DisparoCanhao' atinge alguma coisa, é removido um elemento da lista. Produz o 'Estado' após o efeito de um 'DisparoCanhao'.   
tickCanhao :: [Disparo] -- ^ Lista com, no máximo, dois 'DisparoCanhao'.  
           -> Estado    -- ^ O 'Estado' sem o efeito do 'DisparoCanhao'.
           -> Estado    -- ^ O 'Estado' após o efeito do 'DisparoCanhao' (caso a bala não atinja nada ela é metida na lista de 'Disparo' do 'Estado').
tickCanhao [] e = e
...
tickChoques :: Estado -- ^ O 'Estado' sem os as mudanças nos 'DisparoChoque'.
            -> Estado -- ^ O 'Estado' após as mudanças nos 'DisparoChoque'.
